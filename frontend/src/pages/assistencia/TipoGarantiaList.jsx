...
export default function TipoGarantiaList() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/api/laudos/tipo-garantia');
      setItems(data.items || data || []);
    } catch { toast.error('Erro ao carregar tipos de garantia'); }
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      await api.post('/api/laudos/tipo-garantia', form);
  ...
  return (
    <div className="tarefas-page">
      ...
      {loading ? (
        ...
        <div className="empty-state">Nenhum tipo de garantia cadastrado</div>
      ) : (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Novo Tipo de Garantia"
        footer={
          <>
        ...
        <div className="form-group">
          ...
          <input className="form-control" value={form.descricao}
            ...
            placeholder="Ex: Garantia de fábrica 12 meses" />
        </div>
      </Modal>
    </div>
  );
