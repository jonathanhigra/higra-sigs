...
export default function TipoSensorList() {
  ...
  const load = async () => {
    setLoading(true);
    try { const { data } = await api.get('/api/fabricacao/cadastros/tipo-sensor'); setItems(data); }
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post('/api/fabricacao/cadastros/tipo-sensor', { ...form });
      toast.success('Tipo de sensor criado'); setModal(false); setForm({ descricao: '', sigla: '' }); load();
    } catch { toast.error('Erro ao salvar'); }
  };

  const toggleAtivo = async (item) => {
    try { await api.put(`/api/fabricacao/cadastros/tipo-sensor/${item.id}`, { ativo: item.ativo === 'S' ? 'N' : 'S' }); load(); }
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h1 style={{ margin: 0 }}>Tipo de Sensor</h1>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Novo Tipo de Sensor"
        ...
        <div className="form-group"><label>Sigla</label><input className="form-control" placeholder="ex: PT100" value={form.sigla} onChange={e => setForm(f => ({...f, sigla: e.target.value}))} /></div>
      </Modal>
    </div>
  );
}
